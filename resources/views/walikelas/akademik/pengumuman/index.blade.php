...
<div class="p-0 md:p-6 bg-[#f1f5f9] min-h-screen font-sans">
    ...
    <div class="grid grid-cols-3 gap-2 md:gap-6 mb-8">
        ...
        {{-- Card Status --}}
        <div class="bg-white p-2 md:p-6 rounded-lg shadow-sm border-b-4 border-b-[#004aad] flex flex-col md:flex-row items-center gap-1 md:gap-5 text-center md:text-left">
        <div class="bg-blue-50 p-2 md:p-4 rounded-xl text-[#004aad]">
            ...
            <div>
                ...
                <p class="text-[8px] md:text-lg font-bold text-slate-800">Wali Kelas</p>
            </div>
        </div>
        ...
        <div class="bg-blue-50 p-2 md:p-4 rounded-lg text-[#004aad]">
            ...
            <div>
                ...
                <p class="text-[10px] md:text-2xl font-bold text-slate-800">{{ $pengumumans->total() }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <p class="text-[10px] md:text-lg font-bold text-slate-800">
                    {{ $pengumumans->first() ? \Carbon\Carbon::parse($pengumumans->first()->created_at)->translatedFormat('d F Y') : '-' }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-[0_4px_25px_rgb(0,0,0,0.03)] border border-gray-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-100 bg-white">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6">
                ...
                {{-- Form Filter & Search --}}
                <form action="{{ route('walikelas.pengumuman.index') }}" method="GET" class="flex flex-row items-stretch gap-2">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-8">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-400 overflow-hidden hover:shadow-md hover:border-[#004aad] transition-all duration-300 group flex flex-col h-full">
                    ...
                    <div class="px-5 pb-5 md:px-7 md:pb-7 mt-auto">
                        <a href="{{ route('walikelas.pengumuman.show', $p->id_pengumuman) }}" 
...
<style>
    ...
    /* Penyesuaian Pagination agar serasi dengan desain */
    .pagination nav { display: flex; justify-content: center; }
    .pagination nav svg { width: 1.25rem; }
</style>
@endsection