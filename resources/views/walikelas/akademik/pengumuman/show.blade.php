        ...
        <div class="bg-white rounded-1xl md:rounded-3xl shadow-sm border-1 md:border border-gray-200 overflow-hidden w-full mb-6">
                    ...
                    <div class="bg-white p-2 md:p-5 rounded-xl border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-1 md:gap-4 text-center md:text-left">
                        ...
                        <div>
                            ...
                            <p class="text-[9px] md:text-sm font-bold text-slate-700 uppercase truncate">
                                {{ $pengumuman->target }} {{ $pengumuman->id_kelas ? '- '.$pengumuman->kelas->nama_kelas : '' }}
                            </p>
                        </div>
                    </div>
            ...
            <div class="p-8 md:p-14">
                ...
                <div class="mt-12 p-8 bg-blue-50/50 rounded-2xl border-l-8 border-[#004aad] italic text-sm text-slate-600 shadow-inner">
                    "Harap perhatikan instruksi atau lampiran yang tertera di atas untuk menunjang kegiatan belajar mengajar atau administrasi sekolah."
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-none md:rounded-3xl shadow-sm border-0 md:border border-gray-200 overflow-hidden w-full">
            ...
            <div class="p-4 md:p-14 bg-gray-50/30">
                @if($pengumuman->file_lampiran)
                    ...
                    <div class="bg-white rounded-2xl p-8 border border-gray-100 flex flex-col items-center justify-center min-h-[400px] shadow-sm">
                        @if($isImg)
                        ...
                        @else
                            <div class="text-center p-10">
                                <i class="fa-solid fa-file fa-4x mb-4 text-gray-300"></i>
                                <p class="font-bold uppercase tracking-widest text-xs text-gray-400">Berkas format .{{ $ext }} tersedia</p>
                            </div>
                        @endif
                    </div>
                @else
...
<style>
    ...
    article img { border-radius: 1.5rem; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); max-width: 100%; height: auto; }
</style>
@endsection