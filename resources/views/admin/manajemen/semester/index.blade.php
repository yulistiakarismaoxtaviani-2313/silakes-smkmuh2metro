...
<div class="p-0 md:p-6 bg-[#f1f5f9] min-h-screen font-sans">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 mb-8">
        ...
        <div class="bg-blue-50 w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center text-[#004aad] shadow-sm border border-blue-100 shrink-0">
            ...
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1.5">Total Semester</p>
            <p class="text-xs md:text-base font-bold text-slate-800 uppercase leading-none">{{ $semester->total() }}</p>
            </div>
        </div>
        ...
        <div class="bg-blue-50 w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center text-[#004aad] shadow-sm border border-blue-100 shrink-0">
            ...
            <div class="text-center md:text-left">
            <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1.5">Status Aktif</p>
            <p class="text-xs md:text-base font-bold text-slate-800 uppercase leading-none">
                    {{ $semester->where('status', 'aktif')->first()->nama_semester ?? 'Tidak Ada' }}
                </p>
            </div>
        </div>

        {{-- Shortcut Tambah --}}
        <a href="{{ route('admin.semester.create') }}" class="col-span-2 md:col-span-1 group bg-[#004aad] p-6 rounded-xl shadow-md flex items-center justify-between gap-5 hover:bg-blue-800 transition-all cursor-pointer text-decoration-none">
            <div class="flex items-center gap-5">
                ...
                <div>
                    ...
                    <p class="text-blue-200 text-[10px] uppercase">Klik untuk buat semester baru</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-300 overflow-hidden">
        <div class="p-6 border-b border-gray-300 bg-white flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-bold text-gray-800 uppercase tracking-widest text-sm">Manajemen Data Semester</h2>
            </div>
        </div>
            ...
            <table class="w-full text-sm border-collapse border-spacing-0">
                <thead class="bg-gray-50 border-b-2 border-gray-300">
                    <tr class="text-slate-700 uppercase text-xs tracking-wider">
                        ...
                        <th class="p-3 md:p-4 whitespace-nowrap font-bold border-r border-gray-300">Nama Semester</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors">
                        <td class="p-4 text-center font-reguler text-slate-800 border-r border-gray-300">
                            {{ ($semester->currentPage() - 1) * $semester->perPage() + $loop->iteration }}
                        </td>
                        <td class="p-4 pl-6 text-center border-r border-gray-300">
                            <span class="text-slate-800 font-reguler uppercase text-xs tracking-tight">
                                {{ $item->nama_semester }}
                            </span>
                        </td>
                        <td class="p-4 text-center border-r border-gray-300 whitespace-nowrap">
                            @if($item->status == 'aktif')
                                <span class="bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-lg text-[10px] font-reguler uppercase border border-emerald-200 inline-flex items-center gap-2">
                                    AKTIF
                                </span>
                            @else
                                <span class="bg-gray-100 text-slate-500 px-4 py-1.5 rounded-lg text-[10px] font-reguler uppercase border border-gray-200">
                                    NON-AKTIF
                                </span>
                            @endif
                        </td>
                        <td class="p-4">
                            <div class="flex justify-center gap-2">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('admin.semester.edit', $item->id_semester) }}" 
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('admin.semester.destroy', $item->id_semester) }}" method="POST" class="inline">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus data semester ini?')"
                    ...
                    <tr>
                        <td colspan="4" class="p-20 text-slate-400 italic text-center font-bold tracking-widest uppercase text-xs">
                            Belum ada data semester tersedia
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<style>
    /* Sinkronisasi dengan referensi Siswa */
    .rounded-xl { border-radius: 0.75rem !important; }
    .border-gray-300 { border-color: #cbd5e1 !important; }
    
    /* Styling Pagination agar serasi dengan Dashboard */
