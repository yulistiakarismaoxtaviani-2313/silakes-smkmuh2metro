...
<div class="flex-1 bg-[#F8FAFC] p-0 md:p-8 overflow-y-auto custom-scrollbar font-sans">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-b-xl shadow-xl border-x border-b border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('admin.program-keahlian.update', $pk->id_program_keahlian) }}" method="POST" class="p-10 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="relative group">
                    ...
                    <input type="text" name="nama_program" value="{{ old('nama_program', $pk->nama_program) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="relative group">
                        ...
                        <input type="text" name="kode_program" value="{{ old('kode_program', $pk->kode_program) }}" required maxlength="10"
                            class="w-full border border-gray-200 rounded-xl px-5 py-4 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-[#004AAD]/10 focus:border-[#004AAD] transition-all placeholder:text-gray-300 bg-gray-50/30 uppercase" 
                    ...
                    <div class="relative group">
                        ...
                        <select name="status" required
                            ...
                            <option value="aktif" {{ old('status', $pk->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="nonaktif" {{ old('status', $pk->status) == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
                </div>
