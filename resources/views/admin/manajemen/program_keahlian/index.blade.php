    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 mb-8">
        ...
        <div class="bg-white p-4 md:p-6 rounded-xl shadow-sm border-b-4 border-b-[#004aad] flex flex-col md:flex-row items-center gap-3 md:gap-5">
            ...
            <div class="text-center md:text-left">
                <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1.5">Total Program</p>
                <p class="text-xs md:text-base font-bold text-slate-800 uppercase leading-none">{{ $programKeahlian->total() }}</p>
            </div>
        </div>
        ...
        {{-- Shortcut Tambah --}}
        <a href="{{ route('admin.program-keahlian.create') }}" class="col-span-2 md:col-span-1 group bg-[#004aad] p-6 rounded-xl shadow-md flex items-center justify-between gap-5 hover:bg-blue-800 transition-all cursor-pointer text-decoration-none">
            <div class="flex items-center gap-5">
                <div class="bg-white/20 w-14 h-14 rounded-xl flex items-center justify-center text-white shadow-sm">
                    <i class="fas fa-plus text-2xl"></i>
                </div>
                <div>
                    <p class="text-white font-black uppercase tracking-widest text-sm">Tambah Program</p>
                    <p class="text-blue-200 text-[10px] uppercase">Input Program Baru</p>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-300 overflow-hidden">
        <div class="p-6 border-b border-gray-300 bg-white flex flex-col md:flex-row justify-between items-left md:items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-bold text-gray-800 uppercase tracking-widest text-sm">Daftar Program Keahlian</h2>
            </div>
        </div>
            ...
            <table class="w-full text-sm border-collapse border-spacing-0">
                <thead class="bg-gray-50 border-b-2 border-gray-300">
                    <tr class="text-slate-700 uppercase text-xs tracking-wider">
                        ...
                        <th class="p-3 md:p-4 whitespace-nowrap font-bold border-r border-gray-300">Kode Program</th>
                        <th class="p-3 md:p-4 whitespace-nowrap font-bold border-r border-gray-300">Nama Program</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($programKeahlian as $item)
                    <tr class="hover:bg-blue-50/30 transition-colors">
                        <td class="p-4 text-center font-normal text-slate-800 border-r border-gray-300">
                            {{ ($programKeahlian->currentPage() - 1) * $programKeahlian->perPage() + $loop->iteration }}
                        </td>
                        <td class="p-4 text-center border-r border-gray-300 font-medium text-slate-700">
                            {{ $item->kode_program }}
                        </td>
                        <td class="p-4 pl-6 text-left border-r border-gray-300">
                            <span class="text-slate-700 font-medium uppercase text-xs tracking-tight whitespace-nowrap">
                                {{ $item->nama_program }}
                            </span>
                        </td>
                        <td class="p-4 text-center border-r border-gray-300">
                            <span class="px-2 py-1 rounded-md text-[10px] font-bold uppercase {{ $item->status == 'aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                {{ $item->status }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex justify-center gap-2">
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('admin.program-keahlian.destroy', $item->id_program_keahlian) }}" method="POST" id="form-hapus-{{ $item->id_program_keahlian }}">
                                    @csrf 
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $item->id_program_keahlian }}', '{{ $item->nama_program }}')"
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="p-20 text-slate-400 italic text-center font-bold tracking-widest uppercase text-xs">
                            Belum ada data program keahlian tersedia
                        </td>
                    </tr>
                    @endforelse
                </tbody>
...
<div class="bg-white p-4 border-t border-gray-200 rounded-b-xl">
    <div class="text-[11px] flex items-center gap-3">
        ...
        <div class="text-gray-500 whitespace-nowrap">
            {{ $programKeahlian->firstItem() ?? 0 }} - {{ $programKeahlian->lastItem() ?? 0 }}
            dari
            {{ $programKeahlian->total() }}
        </div>

        <div class="ml-auto custom-pagination">
            {{ $programKeahlian->appends(request()->query())->links() }}
        </div>
    </div>
</div>
...
<script>
    function confirmDelete(id, nama) {
        if (confirm('Apakah Anda yakin ingin menghapus Program Keahlian "' + nama + '"?')) {
...
<style>
    ...
    @media (min-width:768px) {
        ...
        /* Hilangkan layout mobile bawaan Laravel */
